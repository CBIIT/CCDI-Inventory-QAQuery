        ...
        with st, num_p, dg.diagnosis_classification as dg_cancers, count(distinct dg.id) as num_diag

    
        MATCH (st)<-[:of_participant]-(pa:participant)<-[:of_diagnosis]-(diag:diagnosis)
        with st, num_p, num_diag, diag.anatomic_site as dg_sites, count(diag.anatomic_site) as num_sites
        ORDER BY num_sites desc
        with st, num_p, num_diag, collect(dg_sites + ' (' + toString(num_sites) + ')') as sites
        MATCH (st)<-[*..5]-(fl)
        ...
        with st, num_p, num_diag, sites, fl.file_type as ft, count(fl.file_type) as num_ft
        ORDER BY num_ft desc
        with st, num_p, num_diag, sites, collect(ft + ' (' + toString(num_ft) + ')') as file_types, sum(num_ft) as num_files
        ...
        WITH st, num_p, num_diag, sites, file_types, num_files, count(distinct sm1.sample_id) as num_samples_1
        OPTIONAL MATCH (st)<-[:of_participant]-(participant)<-[:of_sample]-(sm1:sample)<--(cp)<--(sm2:sample)
        WHERE (cp:cell_line or cp:pdx)
        WITH st, num_p, num_diag, sites, file_types, num_files, num_samples_1, count(distinct sm2.sample_id) as num_samples_2
        WITH st, num_p, num_diag, sites, file_types, num_files, num_samples_1 + num_samples_2 as num_samples
        MATCH (st)<-[*..5]-(file)
        ...
        WITH st, num_p, num_diag, sites, file_types, num_files, num_samples, file.id as file_id, stf, stp, pub
        RETURN DISTINCT
          st.id as id,
